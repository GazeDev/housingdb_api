...
module.exports = (models) => {
  ...
  return {
    ...
    getLandlordProperties: async function(request, h) {
      ...
      let response;
      try {
        response = await models.Property.findAll(propertyOptions);
      } catch (e) {
        throw Boom.badImplementation('Error during models.Property.findAll(propertyOptions).', e);
      }
    ...
    postLandlord: async function(request, h) {
      ...
      */
      ...
      // revalidate
      try {
        let revalidate = await landlordModels.object.validateAsync(landlord);
      } catch (e) {
    ...
    deleteLandlord: async function(request, h) {
      let response;
      try {
        response = await models.Landlord.destroy({
          where: {
            id: request.params.id,
          },
        });
      } catch (e) {
        throw Boom.badImplementation('Error during models.Landlord.destroy().', e);
      }
      return h
        .response(response)
        .code(202);
    ...
    addLandlordToProperty: async function(request, h) {
      ...
      await landlordInstance.reload();
      let metadata = await landlordInstance.get("metadata");
  ...
  function parseInfoEmail(text) {
    ...
    for (let i=0; i<parts.length; i++) {
      let part = parts[i];
      let result = Joi.string().email().required().validate(part);
  ...
  function parseInfoRemove(string, subject) {
    ...
    let redacted = subject.slice(0, index) + subject.slice(index + string.length)
  	return redacted.replace(/\s+/g, ' ');
  }
