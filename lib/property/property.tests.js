...
lab.experiment('Property', () => {
  ...
  lab.test('PATCH /properties/{propertyId}, admin', async () => {
    const response = await server.inject({
      ...
      payload: {
        'address': '232 North Craig St, Pgh',
      }
    });
    const payload = JSON.parse(response.payload);
    expect(payload.name).to.equal('232 North Craig Street, Pittsburgh, PA 15213');
    expect(payload.PostalAddresses[0].streetAddress).to.equal('232 North Craig Street');
  });
