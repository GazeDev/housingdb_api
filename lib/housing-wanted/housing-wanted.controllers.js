module.exports = (models) => {
    ...
    return {
      ...
      if (request.payload.title && request.payload.title !== request.payload.address) {
        housingWantedObject.title = request.payload.title;
      } 

      if (request.payload.AuthorId) {
        housingWantedObject.AuthorId = request.payload.AuthorId;
      }
      ...
      if (request.payload.contact) {
        housingWantedObject.contact = request.payload.contact;
      }

      if (request.payload.body) {
        housingWantedObject.body = request.payload.body;
      }
      if (request.payload.details) {
        housingWantedObject.details = request.payload.details;
      }
      if (request.payload.status) {
        housingWantedObject.status = request.payload.status;
      }

      let locationId = await locationLib.locationFindCreateNestedTerms(locationTerms);
      housingWantedObject.LocationId = locationId;
