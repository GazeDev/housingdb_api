...
module.exports = {
  ...
  api: Joi.object().keys({
    ...
    price: Joi.number().precision(2).min(0).max(9),
  }),
  apiFilterQuery: Joi.object().keys({
    ...
    bathrooms: Joi.number().precision(2).min(0).max(9).optional(),
    status: Joi.string().optional(),
    price: Joi.number().precision(2).min(0).max(9).optional(),
    locations: [
      Joi.string().guid(),
      Joi.array().items(Joi.string().guid())
    ],
  }).without('search', ['title', 'address', 'bedrooms', 'bathrooms', 'status', 'locations']),
};
