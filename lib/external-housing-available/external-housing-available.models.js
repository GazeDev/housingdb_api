...
module.exports = {
  ...
  apiFilterQuery: Joi.object().keys({
    ...
    beds: Joi.number().integer().min(0).max(10).optional(),
    baths: Joi.number().precision(2).min(0).max(9).optional(),
    status: Joi.string().optional(),
    locations: [
      Joi.string().guid(),
      Joi.array().items(Joi.string().guid())
    ],
  }).without('search', ['title', 'address', 'beds', 'baths', 'status', 'locations']),
};
