module.exports = (models) => {
  ...
  return {
    postExternalHousingAvailable: async function(request, h) {
      ...
      */
      ...
      //belongsTo Account, AuthorId, if it is an admin account then we set else throw
      //use ensureAdmin async/await maybe?
      if (request.payload.AuthorId && request.auth.credentials.subjectId === process.env.SUPER_ADMIN) {
        externalHousingAvailableObject.AuthorId = request.payload.AuthorId;
      } else {
        throw Boom.badRequest('ExternalHousingAvailable.AuthorId must match an Super Admin account ID.');
      }

      //Address, come back to this to see if maybe we use the standardized address.
      if (request.payload.address) {
        let address = request.payload.address
        externalHousingAvailableObject.address = address;
      }
      //Beds
      if (request.payload.beds) {
        //Maybe combine this two line
        let beds = Number(request.payload.beds); //needs to match api 
        externalHousingAvailableObject.beds = beds; //needs to match db
      }
      //Baths
      ...
      if (request.payload.status) {
        ...
        //Maybe have a throw Boom instead saying "Status not set etc."
        externalHousingAvailableObject.status = "N/A";
      }
      ...
      //Setting locationId using location lib function
      ...
      //Property:
      // let propertyId = await propertyLib.getProperty(request.payload.address).PropertyId
      // console.log(propertyId);

      //Go Over this with Zac
      //let propertyInstance = await createProperty(propertyObject);
      // let externalHousingAvailableInstance = await createExternalHousingAvailable(externalHousingAvailableObject);
      
      //belongsTo Property, Account, Location
  ...
  /**
   * addExternalHousingAvailableDetails: async function(request, h) {}
   * Similar to property,landlord metadata. Metadata was declare like above for landlord
   * but nothing for property. 
   */
