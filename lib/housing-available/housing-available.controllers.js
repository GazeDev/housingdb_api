    ...
    postHousingAvailable: async function (request, h) {
      ...
      */
      ...
      try {
        ...
        throw Boom.badImplementation("Housing available instance creation failed.");
      }
      return h
        .response(housingAvailableInstance);
    },
    // Display all Housing Available listings;
    getHousingAvailable: function (request, h) {
      return models.HousingAvailable.findAll()
        .then(response => {
          return response;
        })
        .catch(e => {
          console.log(e);
          throw Boom.badImplementation('Error during getHousingAvailable(request.auth.credentials). ' + e);
        });
    },
    lib: {
      postHousingAvailable: postHousingAvailable,
      getHousingAvailable: getHousingAvailable
    },
  };
